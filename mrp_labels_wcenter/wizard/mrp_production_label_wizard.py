...
class MrpProductionLabelWizard(models.TransientModel):
    ...
    workcenter_id = fields.Many2one(
        "mrp.workcenter", string="Centro de Trabajo Originario", required=True
    )
    qty_labels = fields.Integer(
        string="Número de Etiquetas (Rollos/Cajas)", default=1, required=True
    )

    # Datos de control de peso para la validación en tiempo real
    mo_expected_weight = fields.Float(
        string="Límite por Producir (KG)", digits=(16, 4), readonly=True
    )
    line_ids = fields.One2many(
        "mrp.production.label.wizard.line", "wizard_id", string="Pesaje de Rollos"
    )
    ...
    def _onchange_qty_labels(self):
        ...
        for i in range(self.qty_labels):
            commands.append((0, 0, {"sequence_no": i + 1, "weight": 0.0}))
    ...
    def generate_and_print_labels(self):
        ...
        # Validar peso total
        total_wizard_weight = sum(self.line_ids.mapped("weight"))

        if total_wizard_weight <= 0:
            raise ValidationError(
                _("El peso total de los rollos debe ser mayor a cero.")
            )
        ...
        if total_wizard_weight > self.mo_expected_weight:
            raise ValidationError(
                _(
                    ...
                    total=total_wizard_weight,
                    limit=self.mo_expected_weight,
                )
            )

        Label = self.env["mrp.production.label"]
        ...
        for line in self.line_ids:
            ...
            seq = self.env["ir.sequence"].next_by_code("mrp.production.label.sequence")
        ...
        # Imprime directamente el reporte ZPL (qweb-text)
        return self.env.ref(
            "mrp_labels_wcenter.action_report_production_labels"
        ).report_action(created_labels)
