...
class PackingPrintController(http.Controller):
    ...
    def print_pallet(self, pallet_id, **kw):
        ...
        <div class="label">
          ...
          <div style="margin-top:10px;font-size:9px;line-height:1.2;"><b>{prod_name}</b><br>{cust_name} - {p.customer_code or ''}<br>Operador {p.operator or ''} Maquina {p.machine or ''}</div>
          <div class="barcode"><svg id="bcM"></svg><br><b>{p.name}</b></div>
          <div style="text-align:right;font-size:7px;margin-top:6px;">Impreso por Odoo - {p.operator or ''}</div>
        </div>
    ...
    def print_box(self, box_id, **kw):
        ...
        <div class="label">
          <div class="row"><div class="col">O. Fab/ Mfg No.<br><b>{p.production_id.name}</b></div><div class="col">Cod. Producto/Product No.<br><b>{prod_code}</b></div><div class="col">Rollo Maestro/Master Roll<br><b>{b.master_lot or 'BMC00288264'}</b></div><div class="col">Operador<br><b>{p.operator or ''}</b></div></div>
    ...
    def download_zpl_pallet(self, pallet_id, **kw):
        p = request.env['mrp.pallet'].sudo().browse(pallet_id)
        return request.make_response(p.zpl_pallet or '', [('Content-Type','text/plain'),('Content-Disposition','attachment; filename="%s_MASTER.zpl"' % p.name)])
    ...
    def download_zpl_box(self, box_id, **kw):
        b = request.env['mrp.box'].sudo().browse(box_id)
        return request.make_response(b.zpl_box or '', [('Content-Type','text/plain'),('Content-Disposition','attachment; filename="%s.zpl"' % b.name)])
