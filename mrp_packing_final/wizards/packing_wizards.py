...
class PalletStartWizard(models.TransientModel):
    ...
    def action_next(self):
        ...
        pallet = self.env['mrp.pallet'].create({'production_id':self.production_id.id,'operator':self.operator,'machine':self.machine})
        wizard = self.env['box.entry.wizard'].create({'pallet_id':pallet.id,'production_id':self.production_id.id})
        for i in range(1, self.num_boxes+1):
            self.env['box.entry.line'].create({'wizard_id':wizard.id,'sequence':i,'qty_per_box':2.0,'tara':0.98})
        return {'type':'ir.actions.act_window','name':f'Captura de Cajas - Tarima {pallet.name}','res_model':'box.entry.wizard','res_id':wizard.id,'view_mode':'form','target':'new'}
...
class BoxEntryWizard(models.TransientModel):
    ...
    def action_confirm(self):
        ...
        for line in self.line_ids:
            ...
            self.env['mrp.box'].create({'pallet_id':self.pallet_id.id,'sequence':line.sequence,'master_lot':line.master_lot,'peso_bruto':line.peso_bruto,'peso_neto':line.peso_neto,'tara':line.tara,'qty_per_box':line.qty_per_box,'mill_roll':line.qty_per_box})
        return {'type':'ir.actions.act_window','name':f'Tarima {self.pallet_id.name} - Lista','res_model':'mrp.pallet','res_id':self.pallet_id.id,'view_mode':'form','target':'current'}
